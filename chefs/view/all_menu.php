...
<!-- /Header -->
    ...
    <div class="data md-padding">
      <?php 
        $type;
        $result=$conn->query("SELECT * FROM menu WHERE (menu_id!='1' AND menu_id!='2') ORDER BY menu_type;");
      ...
      <div class="menu shadow-hover" style="">
        ...
        <div class="box-body menu-body bg-theme" id="menu-body" style="background-image: url('../../image/menu/<?php echo $data['menu_img'] ?>');">
          ...
          <?php if ($data['menu_no_obj']=='1') {
            echo '<div class="menu-no-obj-filter" style="display:block;">วัตถุดิบหมด</div>';
          }else {
            echo '<div class="menu-no-obj-filter" style="display:none;">วัตถุดิบหมด</div>';
          } ?>
          ...
          <div class="menu-footer  text-center" ><span class="btn-obj"><?php if ($data['menu_no_obj']!='1') {
            echo '<button class="no_obj" style="width:100%;">วัตถุดิบหมด</button>';            
          }else {
            echo '<button class="have_obj" style="width:100%;">วัตถุดิบมีแล้ว</button>';
          } ?></span></div>
        </div>
      </div>
      <?php 
...
<script>
  ...
  //no obj
 $(document).on('click', '.no_obj', function(event) {
   event.preventDefault();
   var menu_id = $(this).closest('.menu').find('.menu_id').html();
    //alert('ระบบเปลียนสถานะของเมนูแล้ว');
    $(this).closest('.menu').find('.menu-no-obj-filter').show();
    $(this).closest('.btn-obj').html('<button class="have_obj" style="width:100%;">วัตถุดิบมีแล้ว</button>');
    
    $.ajax({
      url: '../controller/manage_menu.php',
      type: 'POST',
      data: {action: 'no_obj', menu_id , menu_id},
    })
    .done(function() {
      //console.log("success");
    })
...
//have obj
  $(document).on('click', '.have_obj', function(event) {
    event.preventDefault();
    var menu_id = $(this).closest('.menu').find('.menu_id').html();
    //alert('ระบบเปลียนสถานะของเมนูแล้ว');
    $(this).closest('.menu').find('.menu-no-obj-filter').hide();
    $(this).closest('.btn-obj').html('<button class="no_obj" style="width:100%;">วัตถุดิบหมด</button>');
