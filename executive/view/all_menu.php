...
<!-- /Background Image -->
        ...
        <div class="row">
          ...
          <div class="col-md-10 col-md-offset-1">
            <div class="myheader-content">
              <h1 class="white-text"> รายการอาหารทั้งหมด</h1>
            </div>
          </div>
          <!-- /home content -->
  ...
  <!-- recommented -->
<?php $result_rcm=$conn->query("SELECT * FROM menu ORDER BY menu_id"); ?>
    ...
    <div class="container">
      ...
      <div class="row">
        ...
        <div class="col-md-4 col-xs-6 menu">
          <img class="img-responsive menu-img" src="../../image/menu/<?php echo $data_rcm['menu_img']; ?>" alt="" style="">
          <div class="overlay">
          ...
          <div class="menu-content">
            ...
            <div class="menu-link"> 
              ...
              <?php if ($data_rcm['menu_id']!='1'&& $data_rcm['menu_id']!='2') { ?>
              
              <span style="display: inline;" class="btn-recom">
                <?php 
                if ($data_rcm['menu_recommented']=='1') {
                  echo '<a class="recom"><i class="fa fa-font-awesome " style="color:yellow"></i></a>';
                }else {
                  echo '<a class="notrecom"><i class="fa fa-font-awesome " ></i></a>';
                } ?>
                
              </span>
              <span style="display: inline;" class="btn-allow">
                <?php 
                if ($data_rcm['menu_status']=='wait') {
...
<script>
  ...
  $(document).on('click', '.allow-menu', function(event) {
    ...
    $(this).closest('.menu').find(".btn-allow").html('<a href="#" class="dont-allow-menu"><i class="fa fa-toggle-on"></i></a>');
  });

  ////dont allow menu //////
  $(document).on('click', '.dont-allow-menu', function(event) {
    event.preventDefault();
    //$(this).closest('.menu').remove();
    var menu_id = $(this).closest('.menu').find('.menu_id').html();
    $.ajax({
      url: '../controller/manage_menu.php',
      type: 'POST',
      data: {menu_id: menu_id,action:'dont-allow'},
    })
    .done(function(data) {
      if(data=='success'){
        //alert("ระบบได้ทำการลบการอนุมัติเมนูดังกล่าวแล้ว !!");
      }
    });
    $(this).closest('.menu').find(".btn-allow").html('<a href="#" class="allow-menu"><i class="fa fa-toggle-off"></i></a>');
  });
  ///////recomment////////
  $(document).on('click', '.recom', function(event) {
    event.preventDefault();
    var menu_id = $(this).closest('.menu').find('.menu_id').html();
     $.ajax({
      url: '../controller/manage_menu.php',
      type: 'POST',
      data: {menu_id: menu_id,action:'not-recom'},
    })
    .done(function(data) {
      if(data=='success'){
        //alert("ระบบได้ทำการลบการอนุมัติเมนูดังกล่าวแล้ว !!");
    ...
    $(this).closest('.menu').find(".btn-recom").html('<a class="notrecom"><i class="fa fa-font-awesome "></i></a>');
  });
  ///////not recomment////////
  $(document).on('click', '.notrecom', function(event) {
    event.preventDefault();
    var menu_id = $(this).closest('.menu').find('.menu_id').html();
     $.ajax({
      url: '../controller/manage_menu.php',
      type: 'POST',
      data: {menu_id: menu_id,action:'recom'},
    })
    .done(function(data) {
      if(data=='success'){
        //alert("ระบบได้ทำการลบการอนุมัติเมนูดังกล่าวแล้ว !!");
    ...
    $(this).closest('.menu').find(".btn-recom").html('<a class="recom"><i class="fa fa-font-awesome " style="color:yellow"></i></a>');
  });
</script>